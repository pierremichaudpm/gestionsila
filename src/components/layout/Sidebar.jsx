...
export default function Sidebar() {
  ...
  return (
    <aside className="flex w-[320px] shrink-0 flex-col bg-[color:var(--color-brand-navy)] text-white">
      ...
      <div className="space-y-1 px-3 py-3">
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="flex items-center gap-2 rounded-lg px-4 py-2.5 text-[15px] text-slate-200 transition-colors hover:bg-white/10"
        >
          <DiscordIcon />
          <span>Discord</span>
          <ExternalIcon />
      ...
      <div className="border-t border-white/10 px-5 py-5">
        ...
        {profile?.country ? (
          <div className="mt-1 text-sm text-slate-300">{profile.country}</div>
        ) : null}
        <button
          type="button"
          onClick={handleSignOut}
          className="mt-4 flex w-full items-center justify-center gap-2 rounded-lg bg-white/15 px-3 py-2.5 text-sm font-semibold text-white transition hover:bg-white/25 hover:ring-2 hover:ring-white/30"
        >
          <LogoutIcon />
          Se déconnecter
        </button>
...
function LogoCircle({ src, alt, cover }) {
  return (
    <div
      title={alt}
      className={[
        'h-[88px] w-[88px] shrink-0 overflow-hidden rounded-full bg-white shadow-sm',
        cover ? '' : 'p-2.5',
      ].join(' ')}
    >
      <img
...
function LogoutIcon() {
  return (
    <svg width="14" height="14" viewBox="0 0 16 16" fill="none" aria-hidden="true">
      <path d="M6 4V3a1 1 0 0 1 1-1h5a1 1 0 0 1 1 1v10a1 1 0 0 1-1 1H7a1 1 0 0 1-1-1v-1M9 8H1m0 0l3-3M1 8l3 3" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  )
}
...
function ExternalIcon() {
  return (
    <svg width="12" height="12" viewBox="0 0 12 12" fill="none" className="ml-auto opacity-60" aria-hidden="true">
